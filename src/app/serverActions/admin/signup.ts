"use server";

import { requestInfo } from "rwsdk/worker";
import { db } from "@/db";
import { initializeServices } from "@/lib/middlewareFunctions";
import { initAuth } from "@/lib/auth";
import { env } from "cloudflare:workers";

function getRedirectUrl(slug: string, request: Request): string {
  const url = new URL(request.url);
  const hostname = url.hostname;
  const protocol = url.protocol;

  if (hostname.includes('localhost')) {
    const port = url.port || '5173';
    return `${protocol}//${slug}.localhost:${port}/dashboard`;
  }

  if (hostname.includes('workers.dev')) {
    const workerDomain = hostname.split('.').slice(-3).join('.');
    return `${protocol}//${slug}.${workerDomain}/dashboard`;
  }

  return `${protocol}//${slug}.${(env as any).APP_URL}/dashboard`;
}

export async function checkUsernameAvailability(username: string) {
  try {
    await initializeServices();

    if (!/^[a-z0-9][a-z0-9\-]*[a-z0-9]$/.test(username)) {
      return { available: false, error: 'Invalid format' };
    }

    if (username.length < 3 || username.length > 32) {
      return { available: false, error: 'Must be 3-32 characters' };
    }

    const reserved = [
      'www', 'api', 'admin', 'app', 'mail', 'ftp', 'localhost',
      'staging', 'dev', 'test', 'demo', 'blog', 'docs', 'support',
      'help', 'status', 'about', 'contact', 'legal', 'privacy',
      'terms', 'signup', 'signin', 'login', 'logout', 'register'
    ];

    if (reserved.includes(username)) {
      return { available: false, error: 'Reserved username' };
    }

    const existingOrg = await db.organization.findUnique({ where: { slug: username } });
    return { available: !existingOrg, username };

  } catch (error) {
    console.error('Error checking username:', error);
    return { available: false, error: 'Error checking availability' };
  }
}

export async function signupWithOrg(formData: FormData) {
  try {
    await initializeServices();

    const { request } = requestInfo;

    const username    = formData.get('username')    as string;
    const displayName = formData.get('displayName') as string;
    const email       = formData.get('email')       as string;
    const password    = formData.get('password')    as string;
    const lairName    = formData.get('lairName')    as string;

    if (!username || !displayName || !email || !password || !lairName) {
      throw new Error('All fields are required');
    }

    if (!/^[a-z0-9][a-z0-9\-]*[a-z0-9]$/.test(username)) {
      throw new Error('Username can only contain lowercase letters, numbers, and hyphens (not at start/end)');
    }

    if (username.length < 3) {
      throw new Error('Username must be at least 3 characters');
    }

    if (password.length < 8) {
      throw new Error('Password must be at least 8 characters');
    }

    const existingOrg = await db.organization.findUnique({ where: { slug: username } });
    if (existingOrg) throw new Error('Username is not available');

    const existingUser = await db.user.findUnique({ where: { email } });
    if (existingUser) throw new Error('Email is already registered');

    // 1. Create user via BetterAuth
    const authInstance = initAuth();
    const signUpResult = await authInstance.api.signUpEmail({
      body: { email, password, name: displayName },
      headers: request.headers
    });

    if (!signUpResult?.user) throw new Error('Failed to create user account');
    const userId = signUpResult.user.id;

    // 2. Create org, then member — clean up org if member creation fails
    const org = await db.organization.create({
      data: {
        id: crypto.randomUUID(),
        name: lairName,
        slug: username,
        metadata: JSON.stringify({ isPersonal: true }),
        createdAt: new Date()
      }
    });

    await db.member.create({
      data: {
        id: crypto.randomUUID(),
        userId,
        organizationId: org.id,
        role: 'owner',
        createdAt: new Date()
      }
    }).catch(async (err) => {
      await db.organization.delete({ where: { id: org.id } });
      throw err;
    });

    const redirectUrl = getRedirectUrl(username, request);

    return {
      success: true,
      redirectUrl,
      user: { id: userId, email: signUpResult.user.email, name: signUpResult.user.name },
      organization: { id: org.id, name: org.name, slug: org.slug },
      session: (signUpResult as any).session
    };

  } catch (error) {
    console.error('❌ Signup error:', error);
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Failed to create account'
    };
  }
}